...
Uint32 get_DDR_ECC_ranges(DDR_ECC_Config * ecc_cfg, 
	DDR_ECC_Addr_Range addressRanges[], unsigned long long ullTotalByteCount)
{
	...
	unsigned int uiTotalByteCount, Bool bECC_enabled)
{
	...
	unsigned int uipreEvtFlag= gpCGEM_regs->EVTFLAG[3];
	...
	//clear false error flag caused by ECC
	if((uipreEvtFlag != gpCGEM_regs->EVTFLAG[3])&&(gpCGEM_regs->EVTFLAG[3]&(1<<(CSL_GEM_MDMAERREVT-96))))
	{
		gpDDR_regs->IRQSTATUS_SYS= CSL_EMIF4F_IRQSTATUS_SYS_REG_WR_ECC_ERR_SYS_MASK;
		gpCGEM_regs->EVTCLR[3]= 1<<(CSL_GEM_MDMAERREVT-96);
	}
...
int KeyStone_DDR_full_leveling()
{
	...
	gpDDR_regs->RDWR_LVL_RMP_CTRL = CSL_EMIF4F_RDWR_LVL_RMP_CTRL_REG_RDWRLVL_EN_MASK;
		// (unsigned int)(1<<CSL_EMIF4F_RDWR_LVL_RMP_CTRL_REG_RDWRLVL_EN_SHIFT);

	/*start full leveling*/
	gpDDR_regs->RDWR_LVL_CTRL = CSL_EMIF4F_RDWR_LVL_RMP_CTRL_REG_RDWRLVL_EN_MASK;
...
{
	Uint32 uiRev = ((gpBootCfgRegs->DEVICE_ID_REG0)&0xF0000000) >> 28;
	...
	unsigned int DDR_PLLD, DDR_ECC_Config * ecc_cfg)
{
	...
	DDR_ECC_Addr_Range addressRanges[3];
	...
	{
		uiEccRangeCnt= get_DDR_ECC_ranges(ecc_cfg, addressRanges, (unsigned long long)(1024*1024*1024));
		if(uiEccRangeCnt)
		{
	...
	while(DDR_INIT_MAX_RETRY_NUM>uiRetryCount)
	{
		...
		for(i=0; i<uiEccRangeCnt; i++)
			Fill_DDR_ECC_range_by_EDMA(
				(Uint32)addressRanges[i].startAddr+0x80000000, /*covert to EDMA virtual address*/
				(Uint32)addressRanges[i].byteCnt);

		//simple test to verify the basic function, return if it pass, retry if it fail
		if(0==DDR_quick_test(0x80000000, 512*1024*1024, (Bool)uiEccRangeCnt))
			return;
